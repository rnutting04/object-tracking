...
# --- GLOBAL STATE ---
# These are global so the mouse callback can access them easily without a class.
offset_x = 0.0
offset_y = 0.0
scale = 100.0   # 1 meter = 100 pixels
dragging_pos = False
last_mouse_pos = (0, 0)
CANVAS_SIZE = 1500  # Internal resolution
...
def draw_scene(w1, h1, w2, h2):
    ...
    # 2. Draw Anchor Plane (Blue - Fixed)
    ...
    # Label Anchor
    ox, oy = anchor_pts[0]
    cv2.putText(img, "Anchor (0,0)", (ox - 20, oy + 40), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 150, 0), 2)

    # 3. Draw Floater Plane (Red - Moving)
...
def get_alignment_offset(w1, h1, w2, h2):
    ...
    Returns: (final_offset_x, final_offset_y)
    """
    ...
    while True:
        ...
        if key == 13: # Enter Key
            break
        elif key == ord('q'): # Quit without saving (optional logic)
