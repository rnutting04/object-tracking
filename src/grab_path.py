...
def world_to_map(X, Y, r_width=1.0, r_height=1.0, img_size=(600, 600), margin=50):
    """
    Converts a point in the world to the pixel coordinates on the map
    """
    h, w = img_size
...
def process_camera(cam_id:str, video_path:str, ref_paths:list, h_inv_path:str, csv_out: str, plot_out: str, r_width=1.0, r_height=1.0):
    ...
    refs = load_references(ref_paths, sift)
    ...
    if not cap.isOpened():
        raise RuntimeError("Could not open video source: {video_path}")
    ...
    # Initalization of variables for processing loop
                ...
                if best_candidate is not None:
                    ...
                    for i, (m, is_inlier) in enumerate(zip(good_matches, matches_mask)):
                        if is_inlier:
                            ...
                            (rx, ry) = ref_data["kp"][m.queryIdx].pt
                            cv2.circle(ref_vis, (int(rx), int(ry)), 3, (0, 255, 0), -1)

                            # Draw on Video Frame
                            (fx, fy) = kp_frame[m.trainIdx].pt
                    ...
                    try:
                        ...
                        if abs(pt_ref_h[2]) > 1e-6:
                            ...
                            cv2.circle(ref_vis, (int(rx), int(ry)), 6, (0, 0, 255), -1)
    ...
    plot_graph(trajectory_smooth, plot_out)

if __name__ == "__main__":
    """
    Script used to run the entire pipeline. The order is as follows:

    Calibrate Homography -> Graph Path -> Fuse Trajectories
